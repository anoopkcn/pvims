...
export const MakeMatName = (material: string) => {
  return reactStringReplace(material, /(\d+)/g, (match, i) => (
    <sub key={i}>{match}</sub>
  ));
};
...
export const makeDataFrom = (datums: number, useR?: boolean) => {
  const length = datums;
  const nullChance = 0;
  const min = 0;
  const max = 100;
...
export const makeDataSubset = (
  ...
  data.filter((item) => {
    ...
    return (
      Number(item?.[key as keyof typeof item]) <= range[1] &&
      Number(item?.[key as keyof typeof item]) >= range[0]
    );
  });
...
export const getDomain = (data: PlotDataType[], key: string) => {
  return [
    data.reduce(
      (min, b) => Math.min(min, Number(b?.[key as keyof typeof b])),
      Number(data[0]?.[key as keyof typeof data[0]])
    ),
    data.reduce(
      (max, b) => Math.max(max, Number(b?.[key as keyof typeof b])),
      Number(data[0]?.[key as keyof typeof data[0]])
    ),
  ];
};
